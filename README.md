...
| 文档 | 内容 |
|---|---|
| [架构设计](docs/architecture.md) | 内部数据结构、对象池、edit-list 算法、反射缓存、锁策略 |
| [性能优化](docs/performance.md) | R04-R13 累计 7+ 项性能优化 + 基准数据 + 高 QPS 清单 |
| [可观测性](docs/observability.md) | 指标 / 慢查询 / 健康检查 / Admin HTTP 端点 / PII 脱敏 / 熔断器 |
| [DAO 迁移指南](docs/migration.md) | 8 步迁移路径 + 11 种模式真实重构示例 |
| [API 参考](docs/api-reference.md) | R04-R13 累计 24+ API 完整公开接口 |
| [R04 轮次总结](docs/rounds/r04-opt/PM.md) | R04 优化轮交付清单 |
| [R13 性能专项](docs/rounds/r13-perf.md) | R11-perf + R13 性能优化 |
...
### R10:`NullInt64` / `NullString` 可空列自动扫描
...
// NULL → 零值 (0 / ""),Valid=false
// 非 NULL → 实际值,Valid=true
// 实现 sql.Scanner + driver.Valuer 双接口
// 工厂: NewNullInt64 / NewNullInt64FromPtr / NewNullString / NewNullStringFromPtr
```
...
// 后期: plugin.SlowQueries() 或 plugin.SlowQueriesJSON() 取快照
...
### R12:Admin HTTP 端点 + Prometheus metrics

```go
// 挂到 wma admin mux 或独立 HTTP server
go http.ListenAndServe(":8080", plugin.AdminHandler())

// 可用端点:
//   GET /debug/stats        → 插件统计 JSON
//   GET /debug/slow         → 最近 100 条慢查询
//   GET /debug/table/{name} → 表结构 JSON
//   GET /metrics            → Prometheus OpenMetrics 格式
```

### R12:鉴权 + 限流(生产推荐配置)

```go
// 鉴权:所有端点要求 X-MySQL-Admin-Token 头
plugin.SetAdminAuthToken(os.Getenv("MYSQL_ADMIN_TOKEN"))

// 限流:令牌桶(默认不限流;设 0,0 显式禁用)
...
```go
// WithMockTx 与 RunInTransaction 等价,行为一致,适合测试场景
mock.ExpectBegin()
mock.ExpectExec("INSERT INTO users").WillReturnResult(sqlmock.NewResult(1, 1))
mock.ExpectCommit()
err := plugin.WithMockTx(ctx, func(tx *MySQLTransaction) error {
    _, err := tx.Exec(ctx, "INSERT INTO users VALUES (?, ?)", 1, "alice")
    return err
})
```
...
`go test -tags integration -count=1 ./...` 跑真实 MySQL 集成路径:
...
覆盖:R04-R12 所有新 API 的真实驱动路径(Start/Stop/CRUD/事务/MapScan/Upsert/BatchExec/BulkUpdate/WithRetry/DescribeTable/Metrics)。
