...
| BUILD_CMD | `go build ./...` |
| BENCH_CMD | `go test -bench=. -benchmem -run=^$ ./... -benchtime=3x` |
...
| r01 | 阶段 1+2 | 风险修复 + 错误统一 | ✅ 完成(待验收) | 11 项改动 + 17 个新单测 |
| r02 | 阶段 3 | 包/分层/命名重构 | 待启动 | package mysql + 6 子包 |
| r03 | 阶段 4 | 测试强化 | 待启动 | 表驱动 + Fuzz + 集成 |
